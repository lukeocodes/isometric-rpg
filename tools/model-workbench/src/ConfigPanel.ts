import type { WorkbenchState, WorkbenchAPI, AnimationMode } from "./WorkbenchAPI";
...
export function createConfigPanel(
  ...
  function buildAnimationControls() {
    ...
    // ─── Mode toggle buttons ───
    const modes: { mode: AnimationMode; label: string; title: string }[] = [
      { mode: "walk",       label: "🚶 Walk",       title: "Walk cycle" },
      { mode: "attack",     label: "⚔️ Attack",      title: "Combat attack swing" },
      { mode: "walk+attack",label: "⚔️🚶 Move+Atk",  title: "Walking while attacking" },
    ];
    const modeGroup = document.createElement("div");
    modeGroup.style.cssText = "display:flex;gap:3px;margin-bottom:6px;flex-wrap:wrap;";
    const modeBtns = new Map<AnimationMode, HTMLButtonElement>();

    for (const { mode, label, title } of modes) {
      const btn = document.createElement("button");
      btn.textContent = label;
      btn.title = title;
      btn.style.cssText = "flex:1;padding:4px 6px;font-size:11px;min-width:60px;";
      const isActive = state.animationMode === mode;
      if (isActive) {
      ...
      btn.addEventListener("click", () => {
        ...
        for (const [m, b] of modeBtns) {
          const active = m === mode;
          b.style.background = active ? "#53a8b6" : "#333";
          b.style.color = active ? "#1a1a2e" : "#aaa";
        }
      });
